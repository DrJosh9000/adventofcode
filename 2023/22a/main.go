...
func main() {
	...
	count := 0
	for _, b := range bricks {
		// We can disintegrate this brick if either:
		// - it supports no other brick, or
		// - any brick it supports is supported by at least one other brick
		removable := true
		for sup := range b.suping {
			if len(sup.supBy) == 1 {
				removable = false
				break
			}
