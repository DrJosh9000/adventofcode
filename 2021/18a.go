...
func main() {
	...
	for _, y := range numbers[1:] {
		x = []any{x, y}
		//log.Printf("after adding: %v", x)
		x = reduce(x)
	}

	fmt.Println(magnitude(x))
}
...
func reduce(x []any) []any {
	for {
		if explode(x) {
			//log.Printf("after explode: %v", x)
			continue
		}
		if s, did := split(x); did {
			x = s.([]any)
			//log.Printf("after split: %v", x)
			continue
		}
		break
	}
...
func explode(x []any) bool {
	...
	//log.Printf("exploding pair at path %v", path)
	...
	//log.Printf("before replacing with 0: %v", x)
